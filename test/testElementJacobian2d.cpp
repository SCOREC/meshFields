...
void setVtxCoords(size_t numVerts, size_t meshDim, TriangleTestCase testTri, ShapeField field) {
...
void triJacobian() {
  ...
  for( auto testCase : {rightTriangle,skewedTriangle} ) {
    ...
    assert(std::fabs(J_h(0,0,0) - testCase.jacobian.at(0)) <= MeshField::MachinePrecision);
    assert(std::fabs(J_h(0,0,1) - testCase.jacobian.at(1)) <= MeshField::MachinePrecision);
    assert(std::fabs(J_h(0,1,0) - testCase.jacobian.at(2)) <= MeshField::MachinePrecision);
    assert(std::fabs(J_h(0,1,1) - testCase.jacobian.at(3)) <= MeshField::MachinePrecision);
    assert(determinants.rank() == 1);
    assert(determinants.extent(0) == 1);
    assert(std::fabs(determinants_h(0) - testCase.jacobianDeterminant) <= MeshField::MachinePrecision);
  }
}
