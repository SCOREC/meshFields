...
struct Functor_TestBatchedTeamVectorQR {
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const MemberType &member) const {
    ...
    /// bb = AA*xx
    KokkosBlas::TeamVectorGemv<MemberType, KokkosBlas::Trans::NoTranspose, Algo::Gemv::Unblocked>::invoke(member, one, aa, xx, zero,
                                                                                              bb);
    member.team_barrier();
    ...
    /// xx = bb;
    TeamVectorCopy<MemberType, KokkosBlas::Trans::NoTranspose, 1>::invoke(member, bb, xx);
    member.team_barrier();

    /// xx = Q^{T}xx;
    TeamVectorApplyQ<MemberType, Side::Left, KokkosBlas::Trans::Transpose, KokkosBlas::Algo::ApplyQ::Unblocked>::invoke(member, aa, tt, xx, ww);
    member.team_barrier();

    /// xx = R^{-1} xx
    TeamVectorTrsv<MemberType, Uplo::Upper, KokkosBlas::Trans::NoTranspose, KokkosBatched::Diag::NonUnit, KokkosBlas::Algo::Trsv::Unblocked>::invoke(
        member, one, aa, xx);
  }
