...
public:
  ...
  void parallel_reduce(FunctorType &reduceKernel, ReductionType &reductionType,
                       std::string tag) {
  ...
  void parallel_for(int lower_bound, int upper_bound, FunctorType &vectorKernel,
                    std::string tag) {
    Cabana::SimdPolicy<vecLen, ExecutionSpace> simd_policy(lower_bound,
                                                           upper_bound);
  ...
  CabSliceController(int n)
    ...
    if (sizeof...(Ts) == 0) {
      throw std::invalid_argument(
          "Must provide at least one member type in template definition");
    }
  }
};
