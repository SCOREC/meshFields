...
public:
  Estimation(Omega_h::Mesh &mesh_in, Omega_h::Reals eps_in,
             ShapeField &eps_star_in, MeshField::Real tolerance_in)
...
template <typename EstimationT, typename OmegahMeshField, typename FieldElement,
          typename ErrorT>
void computeSizeFactor(EstimationT &e, OmegahMeshField &omf,
                       FieldElement &coordFe, ErrorT &errorIntegrator) {
...
void getElementSizeField(EstimationT &e, ErrorT &errorIntegrator) {
  ...
  e.mesh.template add_tag<MeshField::Real>(e.mesh.dim(), "curElmSize", 1,
                                           currentElmSize, false,
                                           Omega_h::ArrayType::VectorND);
  Kokkos::parallel_for(
      e.mesh.nelems(), KOKKOS_LAMBDA(const int elm) {
...
averageToVertex(Omega_h::Mesh &mesh,
                const Kokkos::View<MeshField::Real *> &elmSize) {
  ...
  Kokkos::parallel_for(
      mesh.nverts(), KOKKOS_LAMBDA(const int vtx) {
        MeshField::Real s = 0;
